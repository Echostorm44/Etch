...
internal sealed class ClipParityTests
{
    ...
    private static async Task RunParityTest(string name, SceneBuffer scene)
    {
        ...
        {
            // Impeller references are generated by an external Flutter+Impeller tool and are
            // not committed, so a parity comparison is inconclusive here rather than failing.
            // Skip visibly (regenerate with: cd tools/etch-impeller-ref &&
            // flutter run --enable-impeller -d windows).
            Skip.Test($"Impeller reference PNG not found for {name}: {impellerPngPath}");
            return;
        }
