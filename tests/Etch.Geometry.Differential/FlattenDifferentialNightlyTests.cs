...
internal sealed class FlattenDifferentialNightlyTests
{
    private const double Tolerance = 0.25;

    [Test]
    public async Task Flatten_MatchesKurbo_Cubic_Nightly()
    {
        var runner = new DifferentialRunner(nameof(Flatten_MatchesKurbo_Cubic_Nightly));
    ...
    [Test]
    public async Task Flatten_MatchesKurbo_Quad_Nightly()
    {
        var runner = new DifferentialRunner(nameof(Flatten_MatchesKurbo_Quad_Nightly));
        bool passed = runner.Run(
            iterations: 10_000,
            ...
            property: quad =>
            {
                ...
                {
                    Console.WriteLine($"Flatten Hausdorff exceeded: {hausdorff} > {2 * Tolerance}");
                    Console.WriteLine($"  Quad: P0={quad.P0}, P1={quad.P1}, P2={quad.P2}");
                    Console.WriteLine($"  Ours: {oursSlice.Length} pts, Oracle: {oracle.Count} pts");
                    for (int i = 0; i < Math.Min(oursSlice.Length, 20); i++)
                        Console.WriteLine($"    Ours[{i}] = ({oursSlice[i].X:F4}, {oursSlice[i].Y:F4})");
                    for (int i = 0; i < Math.Min(oracle.Count, 20); i++)
                        Console.WriteLine($"    Oracle[{i}] = ({oracle[i].X:F4}, {oracle[i].Y:F4})");
                }
    ...
    [Test]
    public async Task Flatten_MatchesKurbo_BezPath_Nightly()
    {
        var runner = new DifferentialRunner(nameof(Flatten_MatchesKurbo_BezPath_Nightly));
        bool passed = runner.Run(
            iterations: 10_000,
