...
public static class ColorBurnBlender
{
    ...
    private static float ColorBurnChannel(float s, float d)
    {
        if (s <= 0.0f) return 0.0f;
        return Math.Max(0.0f, 1.0f - (1.0f - d) / s);
    }
    ...
    public static void BlendScalar(ReadOnlySpan<byte> coverage, Rgba16f paint, Span<Rgba16f> row)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            float blendedR = ColorBurnChannel(srcR, dstR);
            float blendedG = ColorBurnChannel(srcG, dstG);
            float blendedB = ColorBurnChannel(srcB, dstB);
