...
public static class HardLightBlender
{
    ...
    private static float HardLightChannel(float s, float d)
    {
    ...
    public static void BlendScalar(ReadOnlySpan<byte> coverage, Rgba16f paint, Span<Rgba16f> row)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            float blendedR = HardLightChannel(srcR, dstR);
            float blendedG = HardLightChannel(srcG, dstG);
            float blendedB = HardLightChannel(srcB, dstB);
